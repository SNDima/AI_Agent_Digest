...
from utils.time_utils import is_search_time_reached, was_search_run_today, should_run_search
...
class TestTimeUtils:
    ...
    def test_should_run_search_conditions_met(self):
        """Test should run search when all conditions are met."""
        with patch('utils.time_utils.datetime') as mock_datetime, \
            ...
            # Last run was yesterday, search time is 12:00 UTC
            last_run = datetime(2023, 12, 31, 8, 0, 0, tzinfo=timezone.utc)  # Yesterday
            result = should_run_search(last_run, "12:00")
            assert result is True

    def test_should_run_search_already_run_today(self):
        """Test should run search when search was already run today."""
        with patch('utils.time_utils.datetime') as mock_datetime, \
             patch('utils.time_utils.logging') as mock_logging:
            # Mock current time to 14:30 UTC on 2024-01-01
            mock_now = datetime(2024, 1, 1, 14, 30, 0, tzinfo=timezone.utc)
    ...
    def test_should_run_search_time_not_reached(self):
        """Test should run search when current time is before search time."""
        with patch('utils.time_utils.datetime') as mock_datetime, \
             patch('utils.time_utils.logging') as mock_logging:
    ...
    def test_should_run_search_no_previous_run(self):
        """Test should run search when there was no previous run."""
        with patch('utils.time_utils.datetime') as mock_datetime, \
             patch('utils.time_utils.logging') as mock_logging:
            # Mock current time to 14:30 UTC on 2024-01-01
            mock_now = datetime(2024, 1, 1, 14, 30, 0, tzinfo=timezone.utc)
            ...
            # No previous run
            result = should_run_search(None, "12:00")
            assert result is True
